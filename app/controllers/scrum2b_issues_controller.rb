class Scrum2bIssuesController < ApplicationController
   # layout nil
  unloadable
  before_filter :find_project, :only => [:index, :board, :update, :update_status, :update_progress, :create, :change_sprint, :close_on_list, :sort,
                                         :close_on_board, :filter_issues_onboard, :filter_issues_onlist]
  before_filter :set_status_settings
  before_filter :check_before_board, :only => [:board, :close_on_board, :filter_issues_onboard, :update, :create]
  ...
  def close_on_board
    ...
    respond_to do |format|
      format.js {
        @return_content = render_to_string(:partial => "/scrum2b_issues/screen_board",:locals => {:id_member => @id_member , :completed_issues => @completed_issues,:project => @project,:new_issues => @new_issues ,
  ...
  def filter_issues_onlist
    ...
    respond_to do |format|
      format.js {
        @return_content = render_to_string(:partial => "/scrum2b_issues/screen_list",:locals => {:sort_versions => @sort_versions,:issues_backlog => @issues_backlog})
      }
      format.html {}
    end
  end
